...
export class Migration20260610030120 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `alter table if exists "email_change_request" drop constraint if exists "email_change_request_token_unique";`
    )
    this.addSql(
    ...
    this.addSql(
      `CREATE UNIQUE INDEX IF NOT EXISTS "IDX_email_change_request_token_unique" ON "email_change_request" ("token") WHERE deleted_at IS NULL;`
    )
    this.addSql(
