...
export class Migration20260601120000 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `CREATE INDEX IF NOT EXISTS "IDX_hop_alert_customer_id" ON "hop_alert" ("customer_id") WHERE deleted_at IS NULL;`
    )
    this.addSql(
      `CREATE INDEX IF NOT EXISTS "IDX_hop_alert_hop_id" ON "hop_alert" ("hop_id") WHERE deleted_at IS NULL;`
    )
    this.addSql(
      `CREATE UNIQUE INDEX IF NOT EXISTS "IDX_hop_alert_customer_hop" ON "hop_alert" ("customer_id", "hop_id") WHERE deleted_at IS NULL;`
    )
    this.addSql(
