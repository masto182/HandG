...
const CheckoutSummary = ({
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mb-8">
        <h2 className="text-hg-gold font-bold text-xl uppercase tracking-tighter">
          Order Summary
        </h2>
        <p className="text-[11px] tracking-wide text-hg-text-secondary uppercase mt-1">
          Collector Tier Member
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-6">
        <div className="space-y-0.5 rounded-lg overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-3 px-3 py-2.5 text-hg-text-secondary/70">
            ...
            <span
              className={`text-[11px] ${shippingRow.isAmount ? "font-semibold" : "italic"}`}
            >
              {shippingRow.text}
            </span>
          </div>
            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  {(() => {
                    const productMeta =
                      (item as any).product?.metadata ||
                      (item as any).variant?.product?.metadata
                    const brewery =
                      productMeta?.brewery_name ||
                      productMeta?.brewery ||
                      (item as any).product_collection ||
                      (item as any).product_subtitle
                    return brewery ? (
                      <p className="text-[10px] text-hg-text-secondary uppercase tracking-wider truncate">
                        {brewery}
                      </p>
                    ) : null
                  })()}
      ...
      <div className="mt-auto pt-6 border-t border-hg-border space-y-3">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="text-hg-text">{shippingRow.text}</span>
        </div>

        <div className="flex justify-between items-center pt-4 border-t border-hg-border">
          <span className="text-sm font-bold text-hg-text uppercase tracking-wide">
            Total
          </span>
