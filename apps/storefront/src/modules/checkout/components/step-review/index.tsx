...
function getBrewery(item: any): string | undefined {
  ...
  return (
    meta?.brewery_name || meta?.brewery || item?.product_collection || undefined
  )
}
...
function shouldShowCarrierCaption(
  displayName: string,
  carrierLabel?: string,
): boolean {
...
const StepReview: React.FC<Props> = ({ cart, isPickup, heatHold }) => {
  ...
  return (
    <div>
      <div className="mb-10">
        <h1 className="text-h1 text-hg-text mb-4">Review Your Order</h1>
          ...
          <div className="p-6 bg-hg-surface border border-hg-border rounded-xl">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold text-hg-text">
                  Shipping To
                </h3>
              </div>
              <a
                href="/checkout?step=address"
                className="text-hg-gold font-semibold text-[11px] hover:underline uppercase tracking-widest"
              >
                Edit
              </a>
            </div>
            <div className="space-y-1 text-sm text-hg-text-secondary">
              <p className="font-bold text-hg-text">
        ...
        <div className="p-6 bg-hg-surface border border-hg-border rounded-xl md:col-span-2">
          ...
          <div className="flex items-center gap-4 text-sm text-hg-text-secondary">
            <div className="h-10 w-16 bg-hl-surface3 rounded flex items-center justify-center border border-hg-border">
              <span className="font-bold text-xs text-hg-text-secondary uppercase">
                PayID
              </span>
            </div>
            <p className="text-hg-text">
              {paymentSession?.provider_id?.includes("payid")
                ? "PayID Transfer"
                : "Cash on Pickup"}
            </p>
          </div>
                ...
                <div className="flex-grow">
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0 flex-1">
                      {brewery && (
                        <p className="text-[10px] text-hg-gold font-semibold uppercase tracking-widest">
                          {brewery}
                        </p>
                      )}
                      <h4 className="text-base font-semibold text-hg-text mt-0.5">
                        {item.product_title}
                      </h4>
                    </div>
                    <span className="font-bold text-base text-hl-price shrink-0">
                      {convertToLocale({
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-3 pt-4 border-t border-hg-border">
        <a
          href="/checkout?step=payment"
      ...
      {heatHold && !isPickup ? (
        <div className="mt-6">
          <HeatHoldBanner
            enabled={heatHold.enabled}
            message={heatHold.message}
          />
        </div>
      ) : null}
    </div>
