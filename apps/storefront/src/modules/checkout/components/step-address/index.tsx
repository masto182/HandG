...
const StepAddress: React.FC<Props> = ({ cart, customer }) => {
  ...
  const [message, rawFormAction] = useActionState(
    async (state: string | null | undefined, fd: FormData) => {
      return setAddresses(
        state,
        setAddressesInputFromFormData(fd),
        fd.get("save_address") === "true",
      )
    },
    null,
  )
  ...
  const iconForAddress = (addr: any) => {
    ...
    if (label.includes("office") || label.includes("work"))
      return (
        <svg
          width="20"
          height="20"
    ...
    <div>
      <div className="mb-10">
        ...
        <p className="text-lg leading-relaxed text-hg-text-secondary">
          Enter your delivery details below. Our temperature-controlled couriers
          ensure your selection arrives in peak condition.
        </p>
      </div>
      ...
      >
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span
                        className={`${isActive ? "text-hg-gold" : "text-hg-text-secondary group-hover:text-hg-gold"} transition-colors`}
                      >
                        {iconForAddress(addr)}
                      </span>
                      <div
                        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${isActive ? "border-hg-gold bg-hg-gold" : "border-hg-border"}`}
                      >
                        {isActive && (
                          <svg
                            width="10"
                    ...
                    <p className="font-semibold text-sm text-hg-text">
                      {(addr.metadata as any)?.label || "Address"}
                    </p>
        ...
        {showForm && (
          <div className="space-y-6">
            <h2 className="text-2xl font-semibold text-hg-text">
              Add New Address
            </h2>
            ...
            {customer && (
              <div className="flex items-center gap-3">
                <input
                  type="hidden"
                  name="save_address"
                  value={saveAddress ? "true" : ""}
                />
                <input
                  type="checkbox"
                  checked={saveAddress}
                  onChange={() => setSaveAddress(!saveAddress)}
                  className="appearance-none w-5 h-5 rounded bg-hg-surface border border-hg-border checked:bg-hg-gold checked:border-hg-gold relative after:content-[''] after:absolute after:inset-0 after:flex after:items-center after:justify-center checked:after:content-['✓'] after:text-[11px] after:text-hg-on-primary after:font-bold after:leading-5 after:text-center cursor-pointer"
                />
