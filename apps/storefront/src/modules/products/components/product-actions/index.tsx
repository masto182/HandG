...
export default function ProductActions({
  ...
  const [restockAlertId, setRestockAlertId] = useState<string | null>(
  ...
  const handleUnsubscribeRestock = async () => {
    ...
    try {
      await sdk.client.fetch(
        `/store/customers/me/restock-alerts/${restockAlertId}`,
        {
          method: "DELETE",
        },
      )
  ...
  return (
    <>
      <div className="flex flex-col gap-y-2" ref={actionsRef}>
        ...
        <div className="p-4 lg:p-6 bg-hg-surface rounded-xl border border-hg-border/30 mt-4">
          {allOutOfStock ? (
