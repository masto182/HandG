...
const AccountNav = ({
  ...
  const isActive = (href: string) => {
    ...
    return (
      routeSegment === href ||
      (href === "/account" && routeSegment === "/account/")
    )
  }
  ...
  const displayName =
    ...
    "Member"
  const tier =
    (customer?.metadata as Record<string, string> | null)?.membership_status ||
    "approved"
  const tierLabel = tier.startsWith("vip")
    ? `VIP Tier ${tier.replace("vip", "")}`
    : "Member"
