...
function getOrderStatus(order: HttpTypes.StoreOrder): string {
  const status =
    (order as any).fulfillment_status || (order as any).status || ""
...
const OrderOverview = ({ orders }: { orders: HttpTypes.StoreOrder[] }) => {
  ...
  return (
    <div className="w-full" data-testid="no-orders-container">
      <div className="border border-outline-variant/30 border-dashed rounded-xl p-6 sm:p-12 flex flex-col items-center text-center">
        ...
        <h2 className="text-h3 font-bold text-on-surface mb-2">
          No orders yet
        </h2>
        <p className="text-body-md text-on-surface-variant max-w-md mb-8">
          Your cellar is currently awaiting its first vintage. Start your
          collection by exploring our curated selection of ultra-limited brewery
          collaborations.
        </p>
        <LocalizedClientLink href="/" passHref>
          <button
            className="bg-primary text-on-primary font-bold px-8 py-4 rounded-lg flex items-center gap-3 active:scale-95 transition-transform hover:brightness-110"
