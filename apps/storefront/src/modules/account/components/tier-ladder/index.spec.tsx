...
describe("TierLadder", () => {
  it("marks the current tier as 'current' and the next as 'next'", () => {
    ...
    expect(screen.getByTestId("tier-row-vip2")).toHaveAttribute(
      "data-state",
      "current",
    )
    expect(screen.getByTestId("tier-row-vip3")).toHaveAttribute(
      "data-state",
      "next",
    )
  ...
  it("marks tiers below current as 'below'", () => {
    ...
    expect(screen.getByTestId("tier-row-approved")).toHaveAttribute(
      "data-state",
      "below",
    )
    expect(screen.getByTestId("tier-row-vip1")).toHaveAttribute(
      "data-state",
      "below",
    )
    expect(screen.getByTestId("tier-row-vip2")).toHaveAttribute(
  ...
  it("marks tiers above next as 'future'", () => {
    ...
    expect(screen.getByTestId("tier-row-vip4")).toHaveAttribute(
      "data-state",
      "future",
    )
    expect(screen.getByTestId("tier-row-vip5")).toHaveAttribute(
      "data-state",
      "future",
    )
  })
  ...
  it("renders perk text from the tier metadata", () => {
    ...
    expect(screen.getByTestId("tier-perk-vip3")).toHaveTextContent(
      "12h Early Access",
    )
    expect(screen.getByTestId("tier-perk-vip5")).toHaveTextContent(
      "2d Priority Access",
    )
  })
