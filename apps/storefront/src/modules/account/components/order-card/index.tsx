...
const OrderCard = ({ order }: OrderCardProps) => {
  ...
  return (
    <div
      className="bg-surface-container rounded-xl border border-outline-variant/20 overflow-hidden transition-all hover:border-primary/30"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <span
                className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded border ${statusBadge.className}`}
              >
                {statusBadge.label}
              </span>
            </div>
          ...
          <div className="sm:text-right">
            ...
            <p
              className="text-price font-bold text-primary"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-end gap-6">
          <div className="flex gap-3">
            {order.items?.slice(0, 3).map((item) => (
              <div
                key={item.id}
                className="w-16 h-16 rounded-lg overflow-hidden border border-outline-variant/30 bg-surface-container-high"
          ...
          <div className="flex gap-3">
            ...
            <LocalizedClientLink href={`/account/orders/details/${order.id}`}>
              <button
                className="px-6 py-3 bg-surface-container-high border border-outline-variant/30 text-on-surface font-bold rounded-lg text-body-sm hover:bg-surface-container-highest transition-colors"
