...
const Overview = ({ customer, orders }: OverviewProps) => {
  ...
  useEffect(() => {
    sdk.client
      .fetch<any>("/store/customers/me/vip", { method: "GET" })
      .then(setVip)
      .catch(() => {})
  }, [])
  ...
  const nextTierLabel = vip?.next_tier
    ? TIER_LABELS[vip.next_tier.tier] || vip.next_tier.tier
    : null
  ...
  const progressPct = vip?.next_tier
    ? Math.min(
        100,
        Math.round(((scoreThreshold - scoreNeeded) / scoreThreshold) * 100),
      )
    : 100
    ...
    <div data-testid="overview-page-wrapper">
      <div className="mb-8">
        ...
        <p className="text-body-md text-hg-text-secondary mt-1">
          {tierLabel && (
            <span className="text-hg-gold font-semibold">{tierLabel}</span>
          )}
          {" · "}
      ...
      <div className="grid grid-cols-1 small:grid-cols-3 gap-4 mb-8">
        <div className="glass-card p-5 rounded-xl border border-hg-border hover:border-hg-gold/30 transition-all">
          ...
          <h3 className="text-label-caps uppercase tracking-[0.05em] text-hg-text-secondary mb-1">
            VIP Score
          </h3>
        ...
        <div className="glass-card p-5 rounded-xl border border-hg-border hover:border-hg-gold/30 transition-all">
          <div className="flex justify-between items-start mb-3">
            ...
            {openOrders > 0 && (
              <span className="text-[10px] font-bold text-hl-accent uppercase tracking-widest">
                Active
              </span>
            )}
          </div>
        ...
        <div className="glass-card p-5 rounded-xl border border-hg-border hover:border-hg-gold/30 transition-all">
          ...
          <p className="text-h2 text-hg-text">
            {vip?.orders_12mo || orders?.length || 0}
          </p>
        </div>
      </div>
        ...
        <div className="glass-card p-6 rounded-xl border border-hg-border mb-8 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              <span className="text-xs font-bold uppercase tracking-widest text-hg-gold">
                Current: {tierLabel}
              </span>
              <div className="h-1 w-1 rounded-full bg-hg-border" />
              <span className="text-xs font-bold uppercase tracking-widest text-hg-text-secondary">
                Next: {nextTierLabel}
              </span>
            </div>
            <h2 className="text-h3 text-hg-text mb-4">
              {scoreNeeded} pts to {nextTierLabel}
            </h2>
            <div className="w-full h-2.5 bg-hg-surface-hover rounded-full overflow-hidden mb-2">
              <div
                className="h-full bg-hg-gold rounded-full transition-all"
            ...
            <div className="flex justify-between text-xs font-medium text-hg-text-secondary">
              ...
              <span>{scoreThreshold} pts total</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 large:grid-cols-2 gap-6">
        <div className="glass-card rounded-xl border border-hg-border flex flex-col">
          <div className="p-5 border-b border-hg-border/50 flex justify-between items-center">
            <h3 className="text-body-md font-semibold text-hg-text">
              Latest Orders
            </h3>
            <LocalizedClientLink
              href="/account/orders"
              className="text-hg-gold text-xs font-bold uppercase hover:underline"
            >
              View All
            </LocalizedClientLink>
          </div>
            ...
            {orders && orders.length > 0 ? (
                ...
                {orders.slice(0, 3).map((order) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className="text-sm font-semibold text-hg-text truncate"
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <Icon
                  name="shopping_bag"
                  size={32}
                  className="text-hg-text-secondary/40 mb-2"
                />
                <span
                  className="text-sm text-hg-text-secondary"
        ...
        <div className="glass-card rounded-xl border border-hg-border flex flex-col">
          ...
          <div className="flex-1 p-5 flex flex-col items-center justify-center py-8 text-center">
            ...
            <LocalizedClientLink
              href="/account/wishlist"
              className="mt-3 text-sm text-hg-gold font-medium hover:underline"
            >
              Go to wishlist →
            </LocalizedClientLink>
          </div>
