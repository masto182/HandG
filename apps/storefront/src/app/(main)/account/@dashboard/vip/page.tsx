...
function formatPerk(hours: number): string {
  ...
  return Number.isInteger(days)
    ? `${days}d Priority Access`
    : `${hours}h Priority Access`
}
...
function buildTiers(
  ...
  return [
    ...
    },
    {
      key: "vip1",
      label: "VIP1",
      points: `${thresholds.vip1 ?? 100} pts`,
      perk: formatPerk(offsets.vip1 ?? 0),
    },
    {
      key: "vip2",
      label: "VIP2",
      points: `${thresholds.vip2 ?? 250} pts`,
      perk: formatPerk(offsets.vip2 ?? 6),
    },
    {
      key: "vip3",
      label: "VIP3",
      points: `${thresholds.vip3 ?? 450} pts`,
      perk: formatPerk(offsets.vip3 ?? 12),
    },
    {
      key: "vip4",
      label: "VIP4",
      points: `${thresholds.vip4 ?? 700} pts`,
      perk: formatPerk(offsets.vip4 ?? 24),
    },
    {
      key: "vip5",
      label: "VIP5",
      points: `${thresholds.vip5 ?? 1000} pts`,
      perk: formatPerk(offsets.vip5 ?? 48),
    },
  ]
}
...
export default async function VipPage() {
  const [data, cfg, membershipStatus] = await Promise.all([
    fetchVipData(),
    getPublicConfig(),
    getMembershipStatus(),
  ])

  if (!data || !isApprovedMember(membershipStatus)) {
    redirect("/account?redirect_to=/account/vip")
  }

  const offsets = (cfg.vip_early_access_offsets_hours as Record<
    string,
    number
  >) || {
    vip1: 0,
  ...
  const nextScore =
    data?.next_tier?.requirements?.score ||
    (nextTierInfo ? thresholds[nextTierInfo.key as keyof typeof thresholds] : 0)
  const progressPercent =
    nextScore > 0 ? Math.min((score / nextScore) * 100, 100) : 100
  const pointsNeeded =
    data?.next_tier?.progress?.score_needed || Math.max(0, nextScore - score)
  ...
  return (
    <div className="w-full" data-testid="vip-page-wrapper">
      ...
      {data?.pending_demotion && (
        <div
          className="flex items-center gap-3 p-4 bg-secondary/10 border border-secondary/30 rounded-xl mb-6"
          data-testid="vip-demotion-warning"
        >
          <svg
          ...
          <p className="text-body-sm text-secondary font-medium">
            Heads up — your score dropped below {currentTierInfo.label}. You
            have {graceDays} days to restore it before you step down a tier.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-surface-container border border-outline-variant/30 rounded-2xl p-6 relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex justify-between items-end mb-4">
                <div>
                  <h3 className="text-h3 font-bold text-on-surface mb-1">
                    Current score: {Math.round(score)}
                  </h3>
                  {nextTierInfo && (
                    <p className="text-body-sm text-primary">
                      Next: {nextTierInfo.label} at {nextScore}
                    </p>
                  )}
                </div>
              ...
              <div className="h-3 w-full bg-surface-container-highest rounded-full mb-4 overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    minWidth: progressPercent > 0 ? "8px" : "4px",
                  }}
                />
              </div>
          ...
          <div
            ...
            data-testid="vip-score-breakdown"
          >
            <h4 className="text-label-caps text-on-surface tracking-wider mb-4">
              SCORE BREAKDOWN (3-MONTH ROLLING)
            </h4>
            <div className="space-y-3">
              <div className="flex justify-between items-baseline border-b border-outline-variant/10 pb-2">
                <div>
                  ...
                  <p className="text-[11px] text-on-surface-variant">
                    1 pt per $1 you spend
                  </p>
                </div>
                <p
                  className="text-body-md font-bold tabular-nums text-on-surface"
                  data-testid="vip-personal"
                >
                  {fmtCurrency(personal)}
                </p>
              </div>
              <div className="flex justify-between items-baseline border-b border-outline-variant/10 pb-2">
                <div>
                  ...
                  <p className="text-[11px] text-on-surface-variant">
                    1 pt per $5 spent by people you referred
                  </p>
                </div>
                <p
                  className="text-body-md font-bold tabular-nums text-on-surface"
                  data-testid="vip-direct"
                >
                  {fmtCurrency(direct)}
                </p>
              </div>
              <div className="flex justify-between items-baseline">
                <div>
                  ...
                  <p className="text-[11px] text-on-surface-variant">
                    1 pt per $10 spent by their referrals
                  </p>
                </div>
                <p
                  className="text-body-md font-bold tabular-nums text-on-surface"
                  data-testid="vip-indirect"
                >
                  {fmtCurrency(indirect)}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
              {[...TIERS].reverse().map((tier, i) => {
                ...
                return (
                  <div
                    key={tier.key}
                    className={`relative flex items-center gap-4 ${isFuture ? "opacity-40" : ""}`}
                  >
                    {isCurrent ? (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        ...
                        >
                          {tier.label}
                          {isCurrent && (
                            <span className="ml-2 text-[10px] font-normal text-primary/70 bg-primary/10 border border-primary/20 px-2 py-0.5 rounded-full align-middle">
                              YOU ARE HERE
                            </span>
                          )}
                          {isNext && (
                            <span className="ml-2 text-[10px] font-normal text-secondary bg-secondary/10 border border-secondary/20 px-2 py-0.5 rounded-full align-middle">
                              NEXT LEVEL
                            </span>
                          )}
                        </span>
                        <span
                          className={`text-label-caps ${isCurrent ? "text-primary" : "text-on-surface-variant"}`}
                        >
                          {tier.points}
                        </span>
                      </div>
      ...
      <footer className="mt-10 pt-8 border-t border-outline-variant/20">
        <div className="flex items-start gap-3 text-on-surface-variant">
          ...
          <p className="text-body-sm italic max-w-2xl">
            Points accumulate on a 3-month rolling window. Tier perks reflect
            current early-access policy and are subject to brewery availability
            and regional shipping laws.
          </p>
        </div>
      </footer>
    </div>
